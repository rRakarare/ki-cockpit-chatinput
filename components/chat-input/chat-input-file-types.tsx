...
export function ChatInputFileDefault({ file }: { file: FileWithMetadata }) {
  return (
    <div className="flex gap-2 justify-between items-center h-full p-2">
      <File className="shrink-0 size-8 border rounded-md bg-red-100 p-2" />
...
export function ChatInputFileXlsx({ file }: { file: FileWithMetadata }) {
  return (
    <div className="flex gap-2 justify-between items-center h-full p-2">
      <FileSpreadsheet className="shrink-0 size-8 border rounded-md bg-emerald-100 p-2" />
      <div className="w-full">
        <span className="line-clamp-1">{file.name}</span>
        <span className="text-xs text-muted-foreground">{file.type}</span>
      </div>
...
export function ChatInputFileDoc({ file }: { file: FileWithMetadata }) {
  return (
    <div className="flex gap-2 justify-between items-center h-full p-2">
      <FileText className="shrink-0 size-8 border rounded-md bg-blue-100 p-2" />
      <div className="w-full">
        <span className="line-clamp-1">{file.name}</span>
        <span className="text-xs text-muted-foreground">{file.type}</span>
      </div>
...
export function ChatInputFileTxt({ file }: { file: FileWithMetadata }) {
  return (
    <div className="flex gap-2 justify-between items-center h-full p-2">
      <FileType className="shrink-0 size-8 border rounded-md bg-accent p-2" />
      <div className="w-full">
        <span className="line-clamp-1">{file.name}</span>
        <span className="text-xs text-muted-foreground">{file.type}</span>
      </div>
